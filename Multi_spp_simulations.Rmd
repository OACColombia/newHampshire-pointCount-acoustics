...
Let's assume that we have sites (point counts of 50m fixed radius) $m = 1,2,~...~,~M=25$ and species $s = 1,2,~...~,S = 20$ in the same area of 100 ha (following the approach in [Lopez et al. 2017](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.12856)). Our latent state $X_{m,s}$ is the site-level log-abundance state for species $s$ at site $m$, which follows a stationary Ornstein-Uhlenbeck, OU, diffusion for Gompertz on log-scale:
...
These expected counts at the point (site) level (0.78 ha, 50m radius) then are extrapolated to the entire area of 100 ha as: $\Lambda_s^{(100)} = \frac{100 \times \lambda_{m,s}^{(0.78)}}{0.78}$  (again, following the approach in [Lopez et al. 2017](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.12856)). 
...
ggplot(lambda_df, aes(x = lambda_counts_078, y = lambda_acoust_078))+
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", linetype = "solid")+
  geom_abline(slope = 1, linetype = "dashed")+
  scale_y_continuous(limits = c(0,100))+
  theme_minimal()+
  labs(x = "Expected mean counts per site\n(point counts)",
       y = "Expected mean counts per site\n(acoustic detections)")+
  coord_fixed()
ggsave(filename = "figs/sim_lambdas.jpg", width = 5, height = 5, units = "in", dpi = 300)
```
![Expected counts from simulated data - from acoustic detections and point counts](figs/sim_lambdas.jpg)

Calibrate $r_s$ (for the overdispersion)
```{r}
...
obs_df %>%
  ...
  summarise(meanCounts = mean(Y_counts),
            ...
            VMR_counts = varCounts/meanCounts, # between 0 and 1, correctly underdispersed!
            meanAcouDet = mean(Y_acoust),
            varAcouDet = var(Y_acoust), 
            VMR_AcouDet = varAcouDet/meanAcouDet)  # >1, correctly overdispersed!

ggplot(obs_df, aes(x = lambda_counts_078,
                 y = lambda_acoust_078))+
  geom_point(alpha = 0.25) +
  geom_point(aes(x = Y_counts,
                 y = Y_acoust), shape = "X", 
             alpha = 0.25) +
  geom_smooth(method = "lm", linetype = "solid")+
  geom_abline(slope = 1, linetype = "dashed")+
  ...
  scale_y_continuous(limits = c(0,100))+
  theme_minimal()+
  labs(x = "Expected (o) and observed (x) counts per site\n(point counts)",
       y = "Expected counts (o) and\nobserved acoustic detections (x) per site")+
  coord_fixed()
ggsave(filename = "figs/sim_lambdas_obs.jpg", width = 5, height = 5, units = "in", dpi = 300)
```
![Expected counts (o) from simulated data of observations (x) - from acoustic detections (Y-axis) and point counts (X-axis)](figs/sim_lambdas_obs.jpg)
